...
| spec_augmentation : random_erase | During each epoch, each CQT array may have a rectangular block of its array values replaced with the value -80 (a low amplitude signal). Visualized as a spectrogram, that means a set number of blocks of a set height and width are blanked out at random locations.  The size of the block is defined by two numbers, the first defining the fraction of the duration of the audio sample, and the second the fraction of the total frequency range of the sample. `prob` specifies the probability of calling the erase method for this feature in this epoch, between 0 and 1. `erase_num` specifies the quantity of such blocks that will be erased if the erase method is called. `region_size` specifies the size of each erased block, as (duration, frequency range) as fractions of the CQT array size, where .01 is 1% and 1.0 is 100%. Default is "[.25, .1]"|
