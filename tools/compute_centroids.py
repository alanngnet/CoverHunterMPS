...
def compute_centroids(embeddings_file: str, output_file: str) -> None:
    ...
    for work_id, perf_ids in work_to_perfs.items():
        ...
        perf_embeds_norm = perf_embeds / np.linalg.norm(perf_embeds, axis=1, keepdims=True)
    ...
    print(f"Saved centroids and radii for {len(centroids)} works to {output_file}")

if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        print("Usage: python compute_centroids.py reference_embeddings.pkl work_centroids.pkl")
        sys.exit(1)
